...
//=================
//Menu
let iconMenu = document.querySelector(".welcome__menu");
let welcLi = document.querySelectorAll(".welcome__li");

document.addEventListener("click", menu);

function menu (event) {
	if (event.target.closest('.welcome__burger')) {
		iconMenu.classList.toggle('_active');
	}
	if (!event.target.closest('.welcome__menu')) {
		iconMenu.classList.remove('_active');
	}
}

for (let i = 0; i < welcLi.length; i++){
welcLi[i].addEventListener("click", function(){
	iconMenu.classList.remove('_active');
});
}
//=================
...
function scroll_scroll() {
	...
	if (header !== null) {
		if (src_value > header.offsetHeight) {
			sticky.classList.add('_scroll');
			header.classList.add('_scroll');
		} else {
			sticky.classList.remove('_scroll');
...
if (goto_links) {
	for (let index = 0; index < goto_links.length; index++) {
		...
		let goto_offset = document.querySelector('.sticky').offsetHeight;
		goto_link.addEventListener('click', function (e) {
			let target_block_class = goto_link.getAttribute('href').replace('#', '');
			let target_block = document.querySelector('.' + target_block_class);
			_goto(target_block, 300, goto_offset);
			e.preventDefault();
		});
	}
}
